...
def _ExtractImportantEnvironment(cpuname):
  ...
  # Get path to a file with variables manually set up to support building for Windows XP/2003
  here = pathlib.Path(script_directory)
  if cpuname == 'x86':
...
def SetupToolchainTargetCombos(vs_path, sdk_version=None, force=True):
  ...
  for (cpu, is_uwp) in itertools.product(cpus, (False, True)):
    ...
    if force or env is None:
      cpuname = cpu
      env = _ExtractImportantEnvironment(cpuname)
